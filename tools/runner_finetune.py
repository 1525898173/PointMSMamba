...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=None):
    ...
    with torch.no_grad():
        ...
        print_log('[Validation] EPOCH: %d  total time = %.4f  batch avg time = %.4f' % (
        epoch, np.sum(val_time), np.mean(val_time)), logger=logger)
        ...
        res_gpu_memory = torch.cuda.max_memory_reserved() / GB
        print_log('[Val GPU Mem] MEM = %.3f GB | Reserved MEM = %.3f GB' % (val_gpu_memory, res_gpu_memory),
                  logger=logger)
