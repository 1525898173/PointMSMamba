...
class get_model(nn.Module):
    def __init__(self, cls_dim, config=None):
        ...
        for i in range(len(self.group_sizes)):
            ...
            self.propagations.append(
                PointNetFeaturePropagation(in_channel=self.trans_dims[i] + 3, mlp=[self.trans_dims[-1] * 4, 1024]))
    ...
    def forward(self, pts, cls_label):
        ...
        for i in range(len(feature_list)):
            feature_list[i] = self.propagations[i](pts.transpose(-1, -2), center_list[i].transpose(-1, -2),
                                                 pts.transpose(-1, -2), feature_list[i].transpose(-1, -2))

        x = torch.cat((feature_list[0], feature_list[1], feature_list[2]), dim=1)  # 3072
        ...
        x_global_feature = torch.cat((x_max_feature + x_avg_feature, cls_label_feature), 1)  # 3072 * 2 + 64
